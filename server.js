...
app.post("/login", async (req, res) => {
	try {
		...
		if (!user) {
			return res.status(401).json({ message: `The user ${user.email} could not be found!` })
		}
...
app.get("/logout", (req, res) => {
	...
	res.status(200).json({ message: `Goodbye ${user.firstName} ${user.lastName}, successfully logged out!` })
})
